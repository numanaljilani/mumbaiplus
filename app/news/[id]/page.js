...
export default async function NewsDetail({ params }) {
  ...
  const pageUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://mumbaiplus.in'}/news/${id}`;
      ...
      <main className="py-8">
        <article className="container mx-auto px-4 max-w-4xl">
          ...
          <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-8">
            <Link href="/" className="hover:text-[#ee73c4] transition">होम</Link>
            <span className="text-gray-400">›</span>
            <Link href="/news" className="hover:text-[#ee73c4] transition">न्यूज़</Link>
            <span className="text-gray-400">›</span>
            <span className="text-[#ee73c4] font-bold">{hindiCategory}</span>
          </nav>

          {/* Header */}
          <header className="mb-10">
            <div className="flex flex-wrap items-center gap-3 mb-5 text-sm">
              <span className="bg-[#ee73c4] text-white px-4 py-2 rounded-full font-bold text-lg">
                {hindiCategory}
              </span>
              ...
              {/* <span className="text-gray-600">{news.readTime || '5'} मिनट पढ़ें</span> */}
            </div>
            ...
            {(news.reporterName || news.userId?.name) && (
              <div className="flex items-center gap-4 mb-8 p-4 bg-pink-50 rounded-2xl">
                <div className="w-16 h-16 bg-[#ee73c4] rounded-full flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {news.ward ? `${news.ward} से रिपोर्टिंग` : 'लोकल रिपोर्टर'}
                  </p>
                </div>
              </div>
            )}
          ...
          {news.tags && news.tags.length > 0 && (
            <div className="flex flex-wrap gap-3 mb-10">
              {news.tags.map((tag, i) => (
                <span
                  key={i}
                  className="bg-pink-100 text-[#ee73c4] px-4 py-2 rounded-full text-sm font-bold"
                >
                  #{tag}
                </span>
              ))}
          ...
          {/* Social Share - खूबसूरत आइकन्स */}
          <div className="bg-gradient-to-r from-[#ee73c4]/10 to-pink-100 rounded-3xl p-8 mb-12">
          ...
          {/* Newsletter */}
          <div className="bg-[#ee73c4] text-white rounded-3xl p-10 text-center">
            <h3 className="text-2xl md:text-3xl font-bold mb-4">
              रोज़ ताज़ा खबरें सीधे आपके इनबॉक्स में
            </h3>
            Conception<p className="text-pink-100 mb-8 text-lg">
            ...
            <form className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              <input
                ...
                required
              />
              <button className="bg-white text-[#ee73c4] px-8 py-4 rounded-full font-bold hover:bg-gray-100 transition shadow-lg">
                सब्सक्राइब करें
              </button>
            </form>
          </div>
...
export async function generateMetadata({ params }) {
  let news = null;
  try {
    const res = await fetch(`http://localhost:5000/api/posts/${params.id}`, { cache: 'no-store' });
